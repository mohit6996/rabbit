...
if (error) return <pp>error{error}</pp>
            ...
            {!orderDetails?<p>NO Order details found</p>:<div className='p-4 sm:p-6 rounded-lg border'>
                ...
                <div className="overflow-x-auto">
                    ...
                    <table className='min-w-full text-gray-600 mb-4'>
                        ...
                        <tbody>
                            {orderDetails.orderItems.map((e,i)=><tr key={e.productId} className='border-b'>
                                <td className='py-2 px-4 flex items-center'>
                                    <img src={e.image} alt={e.name} className='w-12 h-12 object-cover rounded-lg mr-4'/>
                                    <Link to={`/product/${e._id}`} className="text-blue-500 hover:underline">
                                    {e.name}
                                    </Link>


                                </td>
                                <td className="py-2 px-4 ">${e.price}</td>
                                <td className="py-2 px-4 ">${e.quantity}</td>
                                <td className="py-2 px-4 ">${e.price*e.quantity}</td>
                            </tr>)}
                        </tbody>
                    </table>
                </div>
